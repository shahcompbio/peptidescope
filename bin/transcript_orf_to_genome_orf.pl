...
use FindBin;
# path to perllib within singularity image
use lib ("/usr/local/opt/transdecoder/PerlLib");
use Gene_obj;
use GFF3_utils2;
use Data::Dumper;
use Fasta_reader;
...
sub place_orf_in_cdna_alignment_context {
    ...
    if ($orf_orient eq '+') {
        ...
        ## allowing for ORFs on opposite strand of multi-exon transcripts as we have proteomics data to validate with
        ####################
...
#        }
        ...
        if ($trans_orient eq '+') {
            ...
            $WARNING_COUNT++;
            print STDERR "-Warning [$WARNING_COUNT], orf found on opposite strand of transcript. Reorienting transcribed direction accordingly to match orf orient on genome (-).\n";
